...
# Selected areas, not points: `(x, y)` top-left, `(w, h)` size, normalised.
# `PAIR` is the two symmetrical side areas, `CENTER` the middle column.
PAIR = [
    {"x": 0.12, "y": 0.28, "w": 0.30, "h": 0.44},
    {"x": 0.58, "y": 0.28, "w": 0.30, "h": 0.44},
]
CENTER = [{"x": 0.38, "y": 0.30, "w": 0.24, "h": 0.40}]
...
PROTOCOL = [
    ...
    (2, "خون", "این قسمت‌های قرمز، رنگش شبیه خون است.", [{"x": 0.40, "y": 0.06, "w": 0.20, "h": 0.20}], _coding("D", ["C"], "n", ["Bl"]), 0),
    ...
    (7, "دو دختر که به هم نگاه می‌کنند", "صورت‌ها این بالا هستند.", [{"x": 0.22, "y": 0.18, "w": 0.22, "h": 0.26}, {"x": 0.56, "y": 0.18, "w": 0.22, "h": 0.26}], _coding("D", ["M"], "o", ["Hd"], pair=True, popular=True, thematic_codes=["MAH"]), 0),
    ...
    (9, "آتش و دود", "نارنجی‌اش شعله است و سبزش دودی که بالا می‌رود.", [{"x": 0.30, "y": 0.12, "w": 0.40, "h": 0.46}], _coding("D", ["CF", "m"], "u", ["Fi"]), 0),
    (9, "یک صورت عجیب", "دو چشم این‌جاست.", [{"x": 0.40, "y": 0.38, "w": 0.14, "h": 0.16}], _coding("Dd", ["F"], "-", ["(Hd)"]), 1),
    (10, "زیر آب، پر از موجودات رنگی", "رنگ‌های مختلف، مثل خرچنگ و ماهی.", None, _coding("W", ["CF"], "u", ["A", "NC"], synthesis=True), 0),
    (10, "دو خرچنگ آبی", "این قسمت‌های آبی؛ پاهای زیادی دارند.", [{"x": 0.08, "y": 0.20, "w": 0.24, "h": 0.30}, {"x": 0.68, "y": 0.20, "w": 0.24, "h": 0.30}], _coding("D", ["FC"], "o", ["A"], pair=True, popular=True), 0),
    (10, "دو حشره که سر یک چوب دعوا می‌کنند", "هر کدام یک طرف چوب را می‌کشند.", [{"x": 0.40, "y": 0.04, "w": 0.20, "h": 0.18}], _coding("D", ["FM"], "o", ["A"], pair=True, thematic_codes=["AGM"]), 0),
]
