...
> این سند **مرجع کامل** است: هر ۵۰ مسیر و ۵۸ عملیات پیاده‌شده در اینجا آمده‌اند.
...
**`…/content-words/`** — پیشنهاد هوش مصنوعی برای واژه‌های محتوا و دسته‌ی هر پاسخ دور
اول؛ هرگز روی `coding` نمی‌نویسد. `POST` اجرا و ذخیره می‌کند (`{"refresh": true}`
اجرای دوباره را اجبار می‌کند) و `GET` فقط نتیجه‌ی ذخیره‌شده را می‌خواند. پیش از تکمیل
آزمون → `409`؛ `GET` پیش از نخستین اجرا → `404`. شرح کامل، قواعد مهار مدل و شکل
پاسخ در [[14-ai-content-words]].
...
۴۹ مسیر از این ۵۰ مسیر توسط سرویس‌های `Rorschach/src/app/core/api/*` فرانت‌اند مصرف
می‌شوند؛ هیچ صفحه‌ای بدون اندپوینت نیست. تنها استثنا `…/content-words/` است که پس از
تثبیت فرانت‌اند اضافه شده و فعلاً فقط از API و خط فرمان فراخوانی می‌شود
([[14-ai-content-words]] §۱۱).
