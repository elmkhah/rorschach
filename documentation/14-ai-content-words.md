...
مدل با **شناسه‌ی کوتاه** (`R1`، `R2`، …) خطاب می‌شود، نه با UUID پاسخ‌ها. مدل باید هر
شناسه را عیناً پس بدهد و یک UUID گران‌تر از خودِ پیشنهادی است که برچسب می‌زند: روی
پروتکل بیست‌پاسخی، فقط همان UUIDهای تکرارشده جواب را از سقف توکن رد می‌کردند و JSON
وسط کار قطع می‌شد. سقف توکن هم ثابت نیست و از اندازه‌ی پروتکل حساب می‌شود.
...
**انتخاب مدل.** `gpt-4o-mini` مدلی است که این قابلیت با آن سنجیده شده: یک پروتکل
بیست‌پاسخی را در حدود ۹۵۰ توکن و زیر مهلت پیش‌فرض جواب می‌دهد. مدل‌های استدلالی
(`gpt-5-mini` و هم‌خانواده‌هایش) روی همین پروتکل **هم** از مهلت ۲۰ ثانیه رد شدند و
**هم** سقف توکن را خوردند، چون توکن‌های استدلالشان از همین بودجه خرج می‌شود؛ اگر
سراغشان می‌روید `AI_TIMEOUT_SECONDS` و بودجه‌ی توکن را با هم بالا ببرید. در هر دو
حالت خرابی بی‌صدا نیست: تشخیص به واژه‌نامه برمی‌گردد و دلیل را در `error` می‌نویسد.
...
۲۵ تست در `apps/assessments/tests/test_content_words.py`. هیچ‌کدام به شبکه نمی‌روند:
