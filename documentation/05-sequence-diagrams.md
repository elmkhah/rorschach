...
این مستند، الگوهای رفتاری کاربران سیستم را نمایش می دهد.
...
اگر دو تلاش **هم‌زمان** برسند، مسیر سریع هر دو خالی است و هر دو INSERT می‌زنند؛ آن‌گاه قید یکتایی `unique_assessment_client_response` یکی را رد می‌کند و کد، `IntegrityError` را می‌گیرد و رکورد برنده را می‌خواند. یعنی تضمین در **دیتابیس** است نه در منطق برنامه (BR-07).
...
1. **قفل صریح** (`SELECT ... FOR UPDATE`) تا دو کلیک هم‌زمان «ثبت نهایی» دو تحلیل نسازند.
2. **صف پس از commit** است، نه داخل تراکنش. اگر داخل تراکنش بود، worker می‌توانست
   پیش از commit اجرا شود و جلسه را پیدا نکند (BR-10). 3. **مسیر پشتیبان:** اگر worker خاموش باشد، `ensure_analysis()` هنگام نخستین خواندن    پروتکل، تحلیل را همان‌جا محاسبه می‌کند. محاسبه‌ی متغیرهای خام چند ده سطر حساب روی    حداکثر چند ده ردیف است و هزینه‌ای ندارد.
