...
// Paths adapted from Lucide (ISC). Static constants only — safe to trust.
const ICONS = {
...
const DIRECTIONAL = new Set<IconName>(['chevron-left', 'chevron-right', 'arrow-left', 'arrow-right', 'send', 'logout']);
...
@Component({
  ...
  template: `
    <svg
      ...
      [class.rtl:-scale-x-100]="directional()"
      [innerHTML]="svg()"
    ></svg>
  `,
})
export class IconComponent {
  ...
  protected readonly directional = computed(() => DIRECTIONAL.has(this.name()));
}
